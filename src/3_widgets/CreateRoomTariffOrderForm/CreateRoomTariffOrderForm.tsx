...
} from '@/5_entities/roomOrder/api/roomOrderApi.tsx';
...
export const CreateRoomTariffOrderForm = () => {
    ...
    const [createRoomTariffOrder, createResult] =
        useCreateRoomTariffOrderMutation();
    ...
    if (
        id &&
        !isNaN(+id) &&
        !roomTariff.isLoading &&
        roomTariff.status === 'uninitialized'
    ) {
        geyById(+id);
        getBookedDays(+id);
    } else if (!id || isNaN(+id)) {
    ...
    const createRoomTariffOrderHandler = () => {
        ...
        const reqBody: RequestCreateRoomOrderDto = {
            ...
            selected_booked_day: formattedDate,
    ...
    return (
        ...
        >
            ...
            <form method={'POST'}>
                <div
                    className={`h-full md:flex border border-gray-500 rounded-lg p-3 mt-3 gap-4`}
                >
                    <p
                        className={`w-full md:w-2/6 lg:w-3/6 2xl:w-2/6 md:text-left`}
                    >
                        Настройка времени
                    </p>
                    <div
                ...
                >
                    ...
                    <div
                        className={`w-full 2xl:w-4/6 h-full pb-2 flex flex-col justify-center items-stretch bg-white`}
                    >
                        {bookedDays.isSuccess ? (
                            <DatePicker
                                ...
                                onChange={(e) => {
                                    if (e) setSelectedDate(e);
                                }}
                                excludeDates={bookedDays.data!.map(
                                    (i) => new Date(i.date)
                                )}
                        ...
                        <div className={'mb-2'}>
                            <CommonInput
                                placeholder="Доп. время(в минутах)"
                                type={'number'}
                                min={0}
                ...
                {roomTariff.isSuccess ? (
                    <div className={'mt-1 text-gray-700'}>
                        <p className={'leading-5'}>
                            За каждого следующего ребенка:{' '}
                            <strong> {roomTariff!.data.first_price} тг</strong>
                        </p>
                        <p className={'leading-5'}>
                            Продление на 30 минут:{' '}
                            <strong>
                                {' '}
                                {roomTariff!.data.penalty_per_half_hour} тг
                            </strong>
                        </p>
                        <p className={'leading-5'}>
                            Продление на 1 час:{' '}
                            <strong>
                                {roomTariff!.data.penalty_per_hour} тг
                            </strong>
                        </p>
                    </div>
                ) : (
