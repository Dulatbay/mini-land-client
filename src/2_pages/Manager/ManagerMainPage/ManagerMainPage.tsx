...
import {selectRequestParams, setRequestParams} from "@/5_entities/order/model/slice.ts";


// todo: divide
const FilterCheckbox = ({id, label, defaultChecked}: {
    id: string,
    label: string,
                ...
                <div className={'flex gap-8 items-end flex-wrap md:flex-nowrap'}>
                    <div className={'w-full'}>
                        ...
                        <ul className="items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <FilterCheckbox id={"myOrders"} label="Мои заказы" defaultChecked={requestParams.myOrders}/>
                            <FilterCheckbox id="finished" label="Завершенные" defaultChecked={requestParams.finished}/>
                            <FilterCheckbox id="inProcess" label="В процессе" defaultChecked={requestParams.inProcess}/>
                            <FilterCheckbox id="overdue" label="Прошедшие" defaultChecked={requestParams.overdue}/>
                            <FilterCheckbox id="paid" label="Оплаченные" defaultChecked={requestParams.paid}/>
                        </ul>
                    </div>
